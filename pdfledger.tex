...
\includegraphics[width=0.75\textwidth]{./monthbreakdown}\\[1cm]    
...
\verbatiminput{budget.txt}

\chapter{Assets}

\verbatiminput{assets.txt}

\chapter{Liabilities}

\verbatiminput{liabilities.txt}
...
\input{OSUyearbalance.tex}


Balance during \monthname :

\input{OSUmonthbalance.tex}

Transactions for the last 7 days:

\verbatiminput{OSUtrans.txt}

\section{First Tech Credit Union}

Balance over the last year to date :

\input{FirstTechyearbalance.tex}


Balance during \monthname :

\input{FirstTechmonthbalance.tex}

Transactions for the last 7 days:

\verbatiminput{firsttrans.txt}
...
\item The balance of my assets to my liabilities gives my net worth (including retirement funds): \input{networth.txt}
\item Removing long term investment and loan accounts gives my net liquidity: \input{liquidity.txt}
\item Balancing expenses against income yields your cash flow, or net profit/loss(negative is profit, positive is loss): \input{cashflow.txt}
...
\input{OSUforecast.tex}

\verbatiminput{OSUforecast.txt}

\section{First Tech Credit Union}

\input{FirstTechforecast.tex}

\verbatiminput{FirstTechforecast.txt}
